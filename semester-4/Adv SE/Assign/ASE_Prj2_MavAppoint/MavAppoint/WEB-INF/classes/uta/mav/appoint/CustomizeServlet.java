...
public class CustomizeServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response){
		...
		try {
			DatabaseManager dbm = new DatabaseManager();
			if(user.getRole().equalsIgnoreCase("advisor"))
			{
				String cutOffTimeText = request.getParameter("cutOffTimeText");
				AdvisorUser adv = (AdvisorUser) user;
				adv.setCutOffPreference(cutOffTimeText);
				String resp = dbm.updateCutOffTime(adv, cutOffTimeText);
				if(resp != null){
					session.setAttribute("user", user);
